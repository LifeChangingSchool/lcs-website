...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    base("Table 1").select({
        ...
        // if application has already been submitted with this email
        if (records.length > 0 && records[0].get("Submitted") === "true") {
            res.status(200).json({message: "You've already submitted an application with this email address."});
            return;
        }
